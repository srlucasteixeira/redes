...
% Variação segmentada
periodo_slot=ceil(t_quadro/dt_sim)+1;
...
for taxa_quadro=taxa_quadro_vals
	...
	% numero de slots que irá esperar para iniciar nova transmissão
	espera_max =10;
	for r=1:rodadas	
		...
		for t=1:t_sim
            % indica se está iniciando um slot
            inicia_slot= (round(rem(t,periodo_slot))==0);
			...
			for estacao=1:num_estacoes                
			    ...
			    if tx_cnt(estacao)>0
			        ...
			        if (tx_fila(estacao)>0) && (tx_espera(estacao)==0) && (inicia_slot)
			            tx_ativo(estacao)=1;
			            tx_cnt(estacao)=ceil(tam_quadro/taxa_bits/dt_sim);
			            tx_fila(estacao)=tx_fila(estacao)-1;
                    else
                        if tx_espera(estacao)>0 && (inicia_slot)
                ...
                if p_novo < tm_q
                    ...
                    if (tx_ativo(estacao)==0) && (tx_espera(estacao)==0) && (inicia_slot)
                       tx_ativo(estacao)=1;
                       tx_cnt(estacao)=ceil(tam_quadro/taxa_bits/dt_sim);
                    else
                       tx_fila(estacao)=tx_fila(estacao)+1;
...
disp(['e) Duração máxima da janela de tempo de espera aleatória ' num2str(espera_max) ' slots.']) 
