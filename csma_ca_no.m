...
%simulação do protocolo CSMA não-persistente com NÓS OCULTOS considerando
%que o sinal de uma estação só pode ser detectado pelas visinhas mais
%próximas (índice anteriores e seguintes)
    ...
    for a=[0.001]
        ...
        for taxa=1:pontos;
            ...
            for r=1:rodadas
                ...
                for k=1:t_sim
                    % NÃO IMPLEMENTAR ATRASO supondo que RTS e CTS é
                    % realizado
                    % guarda o estado do meio (com atraso )
%                     if k>atraso
...
%                     end
                    ...
                    for j=1:num_estacoes
                        ...
                        % decrementa NAV contador - não tenta gerar novos
                        % pacotes
                        if nav_contador(j)>0
                        ...
                        else % não é colisão durante transmissão
                            
                            %verificar se o quadro está sendo enviado
                            if tx_cnt(j)>0
                                tx_cnt(j)=tx_cnt(j)-1;
                                if tx_cnt(j)==0
                                    tx_ativo(j)=0;
                                ...
                                if (tx_fila(j)>0)
                                    if (tx_espera(j)==0) && (nnz(tx_ativo_atr(jo))==0) && (nav_contador(j)==0)
                                        % pode iniciar uma transmissão - inicia RTS
                                        tx_ativo(j)=1;
                                        tx_cnt(j)=tam_q;
                                        tx_fila(j)=tx_fila(j)-1;
    ...
    disp(['a) Tempo de simulação ' num2str(tempo) '.'])
    ...
    disp(['g) Distância "escutada" ' num2str(d) '.'])
    disp(['h) Colisões ' num2str(colisoes) '.'])
